...
public partial class PomotionCardFactory : CardFactory {
  ...
  protected override CardBase CreateCard(GameState game) {
    ...
    string promotion =
      possiblePromotions[game.GameRandom.RandiRange(0, possiblePromotions.Count - 1)];
    if (promotion == null) {
    ...
    if (validPieceIds.Count == 0) {
      GD.PushError(
        "Tried to make a PromotionCard, but it found no piece ids on the board that didn't match the promotion id.");
      return null;
    }
  ...
  private List<string> GetPossiblePromotions(GameState game) {
    ...
    var possiblePromotions = new List<string>();
    possiblePromotions.AddRange(game.GetAllPieceIds());
    foreach (CardBase card in existingCards) {
